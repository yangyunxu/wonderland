...
#     add data above to database
    for cat, cat_data in cats.items():
        ...
        for p in cat_data['pages']:
            wonder = add_page(c, p['name'], p['rate'], p['intro'])
            add_comment(wonder, "xiaotongwang", '9', 'Great Place!')
            add_comment(wonder, "yuli", '9', 'Amazing Place! Will definitely go there again!')
            add_comment(wonder, "fengqiu", '9', 'Great Place! Stunning View')
            add_comment(wonder, "yunxuyang", '9', 'Great Place! Absolutely worth a try!')
...
def add_cat(name):
    ...
    c.save()
    return c
def add_comment(wonder, name, rate, comment):
    user = User.objects.get_or_create(username=name)[0]
    user_propfile = UserProfile.objects.get_or_create(user=user)[0]
    c = Comment.objects.get_or_create(user=user_propfile, wonder=wonder)[0]
    # c.user = user_propfile
    c.wonder = wonder
    c.rate = rate
    c.comment = comment
    c.save()

def populate_myaccount():
